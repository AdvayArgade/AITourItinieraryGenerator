...
# @st.cache_data() 
def create_page_borders(_doc):
    """
    Creates page borders for all pages in the Word document.
    """
    # Get the section properties element
    sect_pr = _doc.sections[0]._sectPr

    # Create a new element for the page borders
    pg_borders = OxmlElement('w:pgBorders')
    pg_borders.set(qn('w:offsetFrom'), 'page')

    # Set the border style for each side of the page
    for border_name in ('top', 'left', 'bottom', 'right'):
        ...
        # Append the page borders element to the section properties element
    sect_pr.append(pg_borders)

# Function to set background color for header cells
def set_table_header_bg_color(cell):
    ...
    clShading = parse_xml('<w:shd {} w:fill="FFFF99"/>'.format(nsdecls('w')))
...
def text_to_doc(itinerary, input_dict):
    document = Document()
    create_page_borders(document)
    paragraph = document.add_paragraph()
    run = paragraph.add_run()
    run.add_picture("icons/logo.png", width=Inches(5.5))  # Adjust width as needed
    ...
    # Function to add image with specified width
    def add_image(paragraph, image_path, image_width):
        run = paragraph.add_run()
        run.add_picture(image_path, width=image_width)

    # Get the list of all image files from the images folder
    image_files = os.listdir("images")
    # Shuffle the list to get random images
    random.shuffle(image_files)
    # Select top 4 images
    top_4_images = image_files[:4]

    # Create a table to arrange images in a grid
    table = document.add_table(rows=2, cols=2)
    table.alignment = WD_PARAGRAPH_ALIGNMENT.CENTER

    # Add subheader components with images in a grid
    for i, image_path in enumerate(top_4_images):
        row = i // 2
        col = i % 2
        cell = table.cell(row, col)
        cell.width = Inches(2.5)
        add_image(cell.paragraphs[0], f"images/{image_path}", Inches(2.3))
    ...
    table.autofit = False
    table.width = Inches(6)  # Set table width
    table.style = 'Table Grid'  # Apply a predefined table style

    # Set cell widths
    table.columns[0].width = Inches(3)  # Adjust column width
        ...
        for j, cell_data in enumerate(row_data):
            ...
            for paragraph in cell.paragraphs:
                for run in paragraph.runs:
                    run.font.size = Pt(12)
                    run.font.bold = True
                    run.font.color.rgb = RGBColor(0, 0, 0)  # Setting color to black
...
# if st.button("Download Word Document for Hotels and Flight info"):
    # Generate the Word document
    doc = create_word_doc(city_dict, flight_info, input_dict)
